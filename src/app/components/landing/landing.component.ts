...
export class LandingComponent implements OnInit {
  product;
  user = null;
  displayedColumns: string[] = ['productCode', 'status', "price", 'createdAt', 'updatedAt'];
  dataSource;
  ...
  updateBucketList() {
    ...
    return new Promise((resolve, reject) => {
      this.userSrvc.updateUser(id, user).then(
        resp => {
          //////console.log(resp);
  ...
  createOrder() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.unpaidOrder) {
        ...
        this.orderSrvc.createOrder(order).then(
          resp => {
            this.processedOrder = resp;
            resolve(resp)
          }
  ...
  getUnpaidVouchers() {
    ...
    this.orderSrvc.getOrderByUserId(this.user.userId).then(
      resp => {
        ////console.log(resp);
